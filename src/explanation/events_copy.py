...
def get_largest_event_set(
    ...
    return [ (e[1], e[2]) for e in speed_events ]# + time_of_day_events + day_of_week_events
...
def remove_features_by_events(
    ...
    speed_events = events #[tuple(event) for event in events if event[0] == 0]
    ...
    for time_step in range(n_time_steps):
        for node in range(n_nodes):
            ...
            # related to it.
            if (time_step, node) not in speed_events:
