...
class MonteCarloTreeSearch:
    ...
    def __init__(
        ...
        self.expanded_children = expanded_children
    ...
    def reward(
        ...
        for e in input_events:
            ...
            x_subset = scaler.scale(x)
            x_subset = remove_features_by_events(x_subset, e, remove_value=-1)#-2.25)
            x_subsets.append(x_subset)
        x_subsets = torch.stack(x_subsets, dim=0).to(device)
        y = y.repeat(x_subsets.shape[0], 1, 1, 1).to(device)

        # Predict the output graph.
        y_pred = spatial_temporal_gnn(x_subsets)
        ...
        rewards = - mae_criterion(y_pred, y)
        max_reward_idx = torch.argmax(rewards)
        reward = rewards[max_reward_idx].item()
        return max_reward_idx, reward

    def backpropagate(self, path: List[Node], reward: float) -> None:
...
def rollout(monte_carlo_tree: 'MonteCarloTreeSearch', root: Node) -> None:
    """
    ...
    path = monte_carlo_tree._select(root)
    # Get the leaf node.
    return path